...
class FreeTrialStagesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          sl<FreeTrialStagesCubit>()..loadFreeTrialStages(),
      child: const _FreeTrialStagesView(),
    );
  }
}

class _FreeTrialStagesView extends StatelessWidget {
  const _FreeTrialStagesView();
  ...
  Widget _buildSearchBar(BuildContext context, bool isDark) {
    return Container(
      ...
      child: TextField(
        ...
        onChanged: (q) => context.read<FreeTrialStagesCubit>().search(q),
      ),
  ...
  Widget _buildBody(BuildContext context, bool isDark) {
    return BlocBuilder<FreeTrialStagesCubit, FreeTrialStagesState>(
      builder: (context, state) {
        if (state is FreeTrialStagesLoading) return _buildShimmer(isDark);
        if (state is FreeTrialStagesError) return _buildError(context, state.message, isDark);
        if (state is FreeTrialStagesLoaded) {
          if (state.filteredFreeTrialStages.isEmpty) {
            return _buildEmpty(isDark);
          }
          return _buildTable(context, state.filteredFreeTrialStages, isDark);
        }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          FilledButton.icon(
            onPressed: () => context.read<FreeTrialStagesCubit>().loadFreeTrialStages(),
