...
/// FreeTrialSubjects management screen – filtered by gradeId.
class FreeTrialSubjectsScreen extends StatelessWidget {
  final String gradeId;
  const FreeTrialSubjectsScreen({super.key, required this.gradeId});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          sl<FreeTrialSubjectsCubit>()..loadFreeTrialSubjects(gradeId),
      child: _FreeTrialSubjectsView(gradeId: gradeId),
    );
  }
}

class _FreeTrialSubjectsView extends StatelessWidget {
  final String gradeId;
  const _FreeTrialSubjectsView({required this.gradeId});
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            // Breadcrumb
            BlocBuilder<FreeTrialSubjectsCubit, FreeTrialSubjectsState>(
              builder: (context, state) {
                final gradeName =
                    state is FreeTrialSubjectsLoaded ? state.gradeName : '';
  ...
  Widget _buildSearchBar(BuildContext context, bool isDark) {
    return Container(
      ...
      child: TextField(
        ...
        onChanged: (q) => context.read<FreeTrialSubjectsCubit>().search(q),
      ),
    );
  }

  Widget _buildBody(BuildContext context, bool isDark) {
    return BlocBuilder<FreeTrialSubjectsCubit, FreeTrialSubjectsState>(
      builder: (context, state) {
        if (state is FreeTrialSubjectsLoading) return _buildShimmer(isDark);
        if (state is FreeTrialSubjectsError) {
          return _buildError(context, state.message);
        }
        if (state is FreeTrialSubjectsLoaded) {
          if (state.filteredFreeTrialSubjects.isEmpty) return _buildEmpty(isDark);
          return _buildTable(context, state.filteredFreeTrialSubjects, isDark);
        }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () =>
                context.read<FreeTrialSubjectsCubit>().loadFreeTrialSubjects(gradeId),
...
class _FreeTrialSubjectFormDialogState extends State<_FreeTrialSubjectFormDialog> {
