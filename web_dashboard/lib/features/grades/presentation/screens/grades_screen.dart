...
/// Grades management screen – filtered by stageId.
class GradesScreen extends StatelessWidget {
  final String stageId;
  const GradesScreen({super.key, required this.stageId});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          sl<GradesCubit>()..loadGrades(stageId),
      child: _GradesView(stageId: stageId),
    );
  }
}
...
class _GradesView extends StatelessWidget {
  final String stageId;
  const _GradesView({required this.stageId});
          ...
          children: [
            ...
            BlocBuilder<GradesCubit, GradesState>(
              builder: (context, state) {
                final stageName = state is GradesLoaded ? state.stageName : '';
  ...
  Widget _buildSearchBar(BuildContext context, bool isDark) {
    return Container(
      ...
      child: TextField(
        ...
        onChanged: (q) => context.read<GradesCubit>().search(q),
      ),
    );
  }
  ...
  Widget _buildBody(BuildContext context, bool isDark) {
    return BlocBuilder<GradesCubit, GradesState>(
      builder: (context, state) {
        if (state is GradesLoading) return _buildShimmer(isDark);
        if (state is GradesError) return _buildError(context, state.message);
        if (state is GradesLoaded) {
          if (state.filteredGrades.isEmpty) return _buildEmpty(isDark);
          return _buildTable(context, state.filteredGrades, isDark);
        }
  ...
  Widget _buildTable(
      BuildContext context, List<GradeModel> grades, bool isDark) {
    return Container(
      decoration: BoxDecoration(
        color: isDark ? AppColors.cardDark : AppColors.cardLight,
        ...
        child: DataTable2(
          ...
          rows: grades.map((grade) {
            return DataRow2(
              ...
              cells: [
                DataCell(Text('${grade.order}')),
                DataCell(
                  Row(
                    children: [
                      NetworkAvatar(imageUrl: grade.thumbnailUrl, defaultIcon: Icons.class_rounded),
                ...
                DataCell(Text(grade.description ?? '—',
                    overflow: TextOverflow.ellipsis)),
                DataCell(_StatusBadge(isActive: grade.isActive)),
                DataCell(Text('${grade.sectionsCount}')),
                DataCell(Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    _ActionIcon(
  ...
  void _showForm(BuildContext context, {GradeModel? grade}) {
  ...
  void _confirmDelete(BuildContext context, GradeModel grade) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: Text('هل أنت متأكد من حذف "${grade.title}"؟'),
        actions: [
          TextButton(
          ...
          FilledButton(
            onPressed: () {
              Navigator.pop(ctx);
              context.read<GradesCubit>().deleteGrade(grade.id);
            },
