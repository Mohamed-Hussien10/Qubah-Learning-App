...
/// FreeTrialGrades management screen – filtered by stageId.
class FreeTrialGradesScreen extends StatelessWidget {
  final String stageId;
  const FreeTrialGradesScreen({super.key, required this.stageId});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          sl<FreeTrialGradesCubit>()..loadFreeTrialGrades(stageId),
      child: _FreeTrialGradesView(stageId: stageId),
    );
  }
}

class _FreeTrialGradesView extends StatelessWidget {
  final String stageId;
  const _FreeTrialGradesView({required this.stageId});
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            // ── Breadcrumb ───────────────────────────────────────
            BlocBuilder<FreeTrialGradesCubit, FreeTrialGradesState>(
              builder: (context, state) {
                final stageName = state is FreeTrialGradesLoaded ? state.stageName : '';
                return _buildBreadcrumb(context, isDark, stageName);
              },
            ),
  ...
  Widget _buildBreadcrumb(BuildContext context, bool isDark, String stageName) {
    return Row(
      children: [
        InkWell(
  ...
  Widget _buildSearchBar(BuildContext context, bool isDark) {
    return Container(
      ...
      child: TextField(
        ...
        onChanged: (q) => context.read<FreeTrialGradesCubit>().search(q),
      ),
    );
  }

  Widget _buildBody(BuildContext context, bool isDark) {
    return BlocBuilder<FreeTrialGradesCubit, FreeTrialGradesState>(
      builder: (context, state) {
        if (state is FreeTrialGradesLoading) return _buildShimmer(isDark);
        if (state is FreeTrialGradesError) return _buildError(context, state.message);
        if (state is FreeTrialGradesLoaded) {
          if (state.filteredFreeTrialGrades.isEmpty) return _buildEmpty(isDark);
          return _buildTable(context, state.filteredFreeTrialGrades, isDark);
        }
            ...
            return DataRow2(
              ...
              cells: [
                ...
                DataCell(
                  Row(
                    children: [
                      NetworkAvatar(imageUrl: free_trial_grade.thumbnailUrl, defaultIcon: Icons.class_rounded),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('لا توجد صفوف في هذه المرحلة',
              style: TextStyle(
                  color: isDark
                      ? AppColors.textTertiaryDark
                      : AppColors.textTertiaryLight)),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () =>
                context.read<FreeTrialGradesCubit>().loadFreeTrialGrades(stageId),
...
class _FreeTrialGradeFormDialog extends StatefulWidget {
  ...
  @override
  State<_FreeTrialGradeFormDialog> createState() => _FreeTrialGradeFormDialogState();
}

class _FreeTrialGradeFormDialogState extends State<_FreeTrialGradeFormDialog> {
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      _isEditing ? 'تعديل الصف' : 'إضافة صف جديد',
