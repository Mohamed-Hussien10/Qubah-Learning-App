...
/// Units management screen – filtered by subjectId.
class UnitsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          sl<UnitsCubit>()..loadUnits(subjectId),
      child: _UnitsView(subjectId: subjectId),
    );
  }
}
...
class _UnitsView extends StatelessWidget {
  final String subjectId;
  const _UnitsView({required this.subjectId});
          ...
          children: [
            ...
            BlocBuilder<UnitsCubit, UnitsState>(
              builder: (context, state) {
                final subjectName =
                    state is UnitsLoaded ? state.subjectName : '';
                return _buildBreadcrumb(context, isDark, subjectName);
              },
            ),
    ...
    return Row(
      children: [
        ...
        FilledButton.icon(
          ...
          label: Text('${AppStrings.add} وحدة'),
  ...
  Widget _buildSearchBar(BuildContext context, bool isDark) {
    return Container(
      ...
      child: TextField(
        ...
        onChanged: (q) => context.read<UnitsCubit>().search(q),
      ),
    );
  }
  ...
  Widget _buildBody(BuildContext context, bool isDark) {
    return BlocBuilder<UnitsCubit, UnitsState>(
      builder: (context, state) {
        if (state is UnitsLoading) return _buildShimmer(isDark);
        if (state is UnitsError) {
          return _buildError(context, state.message);
        }
        if (state is UnitsLoaded) {
          if (state.filteredUnits.isEmpty) return _buildEmpty(isDark);
          return _buildTable(context, state.filteredUnits, isDark);
        }
  ...
  Widget _buildTable(
      BuildContext context, List<UnitModel> units, bool isDark) {
    return Container(
      decoration: BoxDecoration(
        color: isDark ? AppColors.cardDark : AppColors.cardLight,
      ...
      child: ClipRRect(
        ...
        child: DataTable2(
          ...
          columns: const [
            ...
            DataColumn2(label: Text('الدروس'), fixedWidth: 90),
          ...
          rows: units.map((unit) {
            return DataRow2(
              ...
              cells: [
                ...
                DataCell(Text('${unit.lessonsCount}')),
                DataCell(Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    _ActionIcon(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('لا توجد وحدات في هذه المادة',
              style: TextStyle(
                  color: isDark
                      ? AppColors.textTertiaryDark
                      : AppColors.textTertiaryLight)),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () =>
                context.read<UnitsCubit>().loadUnits(subjectId),
  ...
  void _showForm(BuildContext context, {UnitModel? unit}) {
    ...
    showDialog(
      ...
      builder: (_) => _UnitFormDialog(
        unit: unit,
        subjectId: subjectId,
        onSave: (u, {imageBytes, imageName}) async {
          if (unit != null) {
            await cubit.updateUnit(u, imageBytes: imageBytes, imageName: imageName);
          } else {
            await cubit.createUnit(u, imageBytes: imageBytes, imageName: imageName);
          }
        },
      ),
    );
  ...
  void _confirmDelete(BuildContext context, UnitModel unit) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('هل أنت متأكد من حذف "${unit.title}"؟'),
        actions: [
          TextButton(
...
class _UnitFormDialog extends StatefulWidget {
  ...
  final String subjectId;
  final Future<void> Function(UnitModel, {List<int>? imageBytes, String? imageName}) onSave;
  const _UnitFormDialog(
      {this.unit, required this.subjectId, required this.onSave});
  @override
...
class _UnitFormDialogState extends State<_UnitFormDialog> {
  ...
  void initState() {
    ...
    _thumbCtrl = TextEditingController(text: widget.unit?.thumbnailUrl ?? '');
    _orderCtrl =
        TextEditingController(text: widget.unit?.order.toString() ?? '0');
  ...
  Future<void> _handleSave() async {
    ...
    final unit = UnitModel(
      ...
      lessonsCount: widget.unit?.lessonsCount ?? 0,
      createdAt: widget.unit?.createdAt,
    );
    try {
      await widget.onSave(unit, imageBytes: _selectedFileBytes, imageName: _selectedFileName);
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  Text(_isEditing ? 'تعديل الوحدة' : 'إضافة وحدة جديدة',
